...
if has("gui_running") && has("menu") && &go =~ 'm'
 ...
 exe 'menu '.g:DrChipTopLvlMenu.'DrawIt.Start\ DrawIt<tab>\\di		<LocalLeader>di'
endif

" ---------------------------------------------------------------------
" Public Interface: {{{1
if !hasmapto('<Plug>StartDrawIt')
  map <unique> <LocalLeader>di <Plug>StartDrawIt
endif
...
if !hasmapto('<Plug>StopDrawIt')
  map <unique> <LocalLeader>ds <Plug>StopDrawIt
endif
map <silent> <Plug>StopDrawIt :set lz<cr>:call DrawIt#StopDrawIt()<cr>:set nolz<cr>:source ~/.vim/plugin/boxdraw.vim<cr>:call Source_comma_map()<cr>
